...
<script src="https://connect.facebook.net/ko_KR/sdk.js?hash=905682cda7de5864b5db64fe6e68d597" async="" crossorigin="anonymous"></script><script id="facebook-jssdk" src="//connect.facebook.net/ko_KR/sdk.js"></script><script async="" src="//www.google-analytics.com/analytics.js"></script><script>
			...
			<div id="top_MSR" class="eFreqTop"><!-- 20170510 수정 : 클래스 eFreqTop 추가 -->
				<div class="top_msr_wrap" style="display:none;">
					...
					<div class="msr_card_zone m_card_pic">
						...
						<div class="mycard_one" style="transform: rotateY(359.667deg);">
							<!-- <div class="front"> -->
                    ...
                    <div class="product_view_wrap1">
                        ...
                        <div class="product_view_detail">
                            ...
                            <div class="m_view_slide">
                                ...
                                <p id="prevBtn" style="display: none;"><a href="javascript:void(0)" role="button">이전 버튼</a></p><!-- 접근성_20171123 role 추가 -->
                                <p id="nextBtn" style="display: none;"><a href="javascript:void(0)" role="button">다음 버튼</a></p><!-- 접근성_20171123 role 추가 -->
                            </div>
                            <form method="post">
                                <fieldset>
                                    ...
                                    <div class="product_view_info">
                                        ...
                                        <div class="product_sns_wrap">
                                            <ul class="product_sns">
                                                ...
                                                <li data-sns="F"><a href="javascript:void(0)" title="페이스북 공유하기 새창"><!-- 접근성_20171123 title 추가 --><img src="//image.istarbucks.co.kr/common/img/menu/sns02.png" alt="페이스북 공유하기"><!-- 접근성_20171123 alt 추가 --></a></li>
                                            </ul>
                                        </div>
                                    </div>
					...
					<div class="productView_footmenu_inner">
						...
						<div class="productView_top">
							...
							<div class="productView_top_right">
								<dl>
									<dt>
										<p class="sirenOrder"><a href="util/app_tip"><img src="//image.istarbucks.co.kr/common/img/menu/sirenOrder_img_171109.png" alt="사이렌오더"></a></p>
									</dt>
									<dd>
		...
		<script>
			...
			function getDataInfo() {
                ...
                function(data) {
                    ...
                    // 영양정보
                    $('.product_info_content dd').eq(0).text(convertInfoText(data.view.kcal));
                    $('.product_info_content dd').eq(1).text(convertInfoText(data.view.fat));
                    $('.product_info_content dd').eq(2).text(convertInfoText(data.view.sat_FAT));
                    $('.product_info_content dd').eq(3).text(convertInfoText(data.view.trans_FAT));
                    $('.product_info_content dd').eq(4).text(convertInfoText(data.view.cholesterol));
                    $('.product_info_content dd').eq(5).text(convertInfoText(data.view.sugars));
                    $('.product_info_content dd').eq(6).text(convertInfoText(data.view.chabo));
                    $('.product_info_content dd').eq(7).text(convertInfoText(data.view.protein));
                    $('.product_info_content dd').eq(8).text(convertInfoText(data.view.sodium));
                                        
                    //[151110] 푸드인 경우 영양정보 특정 항목만 출력 함
                    /* switch (data.view.cate_NAME) {
                    	case "브레드":
                    	case "케익":
                    	case "샌드위치&샐러드":
                    	case "따뜻한 푸드":
                    	...
                    	case "기타 푸드":
                    	case "2016 오텀1 프로모션":
                    	case "2016 크리스마스1 프로모션":
                    	case "2016 서머3 프로모션":
                    		$('.product_info_content li').eq(6).remove();
                    		$('.product_info_content li').eq(4).remove();
                    		$('.product_info_content li').eq(3).remove();
                    		$('.product_info_content li').eq(1).remove();
                    		$('.product_info_content ul').eq(0).append( $('.product_info_content li').last().addClass("last") );
                    		break;
                    	default:
		                    //[160520] 특정 상품 일부 영양정보 숨김 (해당 상품들의 카테고리명은 위에 없음..)
		                    if ("9300000000068|9300000000110|9300000000108|9300000000109|5120007089|5120007090|9300000000067|5110003081|5110003074|5110003082|5110003056".indexOf($PRODUCT_CD) > -1) {
		                    	$('.product_info_content li').eq(6).hide();
		                    	$('.product_info_content li').eq(4).hide();
		                    	$('.product_info_content li').eq(3).hide();
		                    	$('.product_info_content li').eq(1).hide();
		                    	$('.product_info_content ul').eq(0).append( $('.product_info_content li').last().addClass("last") );
		                    }
                    		break;
                    } */
                    ...
                    if(data.view.allergy == "") {
                        $('div.product_factor').hide();
                    } else {
                        $('div.product_factor > p').text('알레르기 유발요인 : ' + data.view.allergy.replace(/,/gi, ' /').replace(/@/gi, ' / '));
                    }
                    ...
                    $('.product_sns_wrap li').on('click', function() {
                    	...
                    	if(snsType == 'T') {
                            var $param = {
                                ...
                                description : data.view.product_NM + "\n" + m_domain_http + "menu/food_view?product_cd=" + $PRODUCT_CD
                            };
                            twitterShare( $param );
                    	} else if(snsType == 'F') {
                            ...
                            var $param = {
                                ...
                                link: m_domain_http + "menu/food_view?product_cd=" + $PRODUCT_CD,
                                image: $('.product_thum li:eq(0) img').attr('src'),
                                caption: '스타벅스 코리아',
            ...
            function getFileInfo(product_cd) {
                ...
                function(data) {
                    ...
                    $('.elevatezoom-gallery').eq(0).addClass('active');
                    
                    $('.zoomImg').attr('src', 'https://image.istarbucks.co.kr' + data.file[0].file_PATH);
                    $('.zoomImg').attr('data-zoom-image', 'https://image.istarbucks.co.kr' + data.file[0].file_PATH);

                    $('#tpl_productMobLi').tmpl(data.file).appendTo($('.m_view_slider'));

                    var m_slider = $('.m_view_slider').bxSlider({
                        auto: false,
                        pager: true,
                        controls: true,
                        autoControls: true,
                        autoControlsCombine: true,
                        pagerSelector: '.m_view_pagers',
                        autoControlsSelector: '.m_view_controls'
                    });
...
<script>
	...
	$(document).ready(function () {
		// 현재 categoryType 획득 (01:음료, 02:Food, 05:원두/기타)
		if (location.href.indexOf("coffee") > -1 && location.href.indexOf("product") > -1 ) {
			m_categoryType = "05";
			m_cateTypeText   = "원두";
			coffee_url = "coffee/product_view?PRODUCT_CD=";
		}else if(location.href.indexOf("coffee") > -1 && location.href.indexOf("reserve") > -1 ){
			m_categoryType = "05";
			m_cateTypeText   = "리저브™";
