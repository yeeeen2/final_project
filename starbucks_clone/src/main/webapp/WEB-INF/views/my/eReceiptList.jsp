...
var eFrequencyPlannerYn = 'Y';
		...
		<link href="../common/css/style_receipt.css?v=190604" rel="stylesheet">
		<link href="../common/css/style_mem.css" rel="stylesheet">
		<link href="../common/css/style_util2.css" rel="stylesheet"><!-- 20181106 추가 --> 
	</head>
	<body>
		...
		<!-- 서브 gnb end -->
			...
			<div id="container">
				...
				<header class="ms_sub_tit_wrap">
					<div class="ms_sub_tit_bg">
						<div class="ms_sub_tit_inner">
							<!-- 160609 텍스트 수정 -->
							<h4><img alt="보유 카드" src="//image.istarbucks.co.kr/common/img/util/receipt_ttl.png"></h4>
							<!-- 160609 텍스트 수정 end -->
							<ul class="smap">
								...
								<li><img class="arrow" src="//image.istarbucks.co.kr/common/img/common/icon_arrow_w.png" alt="하위메뉴"></li>
								<!-- 160609 텍스트 수정 -->
								<li><a href="/my/eReceiptList.do">전자영수증</a></li>
								<!-- 160609 텍스트 수정 end -->
							</ul>
						</div>
					</div>
				...
				<div class="ms_cont_wrap">
					<div class="ms_cont">
						<!-- 기간선택 -->
						<!-- <section class="my_card_pick_period m0">
									...
									<dl class="my_card_pick_date">
										...
										<dd>
											<div class="pick_date_each">
												<div class="pick_date_each_left"><input id="receiptCal1" name="receiptCal" title="원하는 날짜를 선택해 주세요." type="text"></div>
												<div class="pick_date_each_right"><label for="receiptCal1">날짜 선택</label></div>
											</div>
											<p class="hyphen_bg"></p>
											<div class="pick_date_each">
												<div class="pick_date_each_left"><input id="receiptCal2" name="receiptCal" title="원하는 날짜를 선택해 주세요." type="text"></div>
												<div class="pick_date_each_right"><label for="receiptCal2">날짜 선택</label></div>
											</div>
											<p class="btn_pick_date"><a href="javascript:makeReceiptPage(1);">검색</a></p>
										</dd>
									</dl>
									
									<p class="my_card_infoTxt">&nbsp;*최근 5년까지의 이력만 조회 가능합니다.</p>
								</fieldset>
							</form>
						</section> -->
						<!-- 기간선택 end -->
						...
						<!-- 목록 -->
						<!-- <p class="receipt_tb_notice mb10">매장에 방문하여 거래하신 건에 한하여 전자 영수증을 <br class="for_mob"/>확인하실 수 있습니다.</p> 20171017수정 : 클래스 추가 -->
						<table class="card_use_info_tbl" summary="my카드 사용 구분, 내역, 이용매장, 금액, 날짜에 대한 테이블">
							...
							<tbody>
								<c:set var="j" value="${listSize }"/>
								<c:set var="i" value="0"/>
								<c:forEach var="receiptDTO" items="${list }">
									<tr>
										<td>${j}</td>
										<td>${dateList[i] }</td>
										<td>${receiptDTO.p_name }</td>
										<td>${receiptDTO.salecount }</td>
										<td>${receiptDTO.price }</td>
										<c:if test="${receiptDTO.salemethod=='kakao'}">
											<td>카카오페이</td>
										</c:if>
										<c:if test="${receiptDTO.salemethod=='sbcard'}">
											<td>스타벅스 카드</td>
										</c:if>
									</tr>
								...
								<%-- <tr>
									<td colspan="6" style="text-align: center;">${page }</td>
								</tr> --%>
							</tbody>
						</table>
...
// 					});
			...
			<script>
				...
				var serverTime = 20220707133012;
				
				
				$(document).ready(function(){
					//$("input[name=pickPeriod][value=P]").prop("checked", true);
					$('input[name=paperYn]').ezMark();

					//이벤트 설정
					setEvent();
					
					
					$("input[name=pickPeriod]").eq(0).prop("checked", true).trigger("change");
				...
				//전자영수증 페이지 셋팅
				function makeReceiptPage(pageNum){
					...
					//캘린더 날짜 유효성 검사
					if(pattern_check("#receiptCal1", "올바르지 않은 날짜 형식입니다." , "올바르지 않은 날짜 형식입니다.", getPattern('DATE')) == false){return;}
					if(pattern_check("#receiptCal2", "올바르지 않은 날짜 형식입니다." , "올바르지 않은 날짜 형식입니다.", getPattern('DATE')) == false){return;}
					
					//전자영수증 API 호출
					var objParam = {
						 searchType   : $("input[name=pickPeriod]:checked").val()
						,startDt      : $("#receiptCal1").val()
						,endDt        : $("#receiptCal2").val()
						,payMethod    : $('#myinfo_date01 option:selected').val()
						,transactType : $('#myinfo_date02 option:selected').val()
						,pagingSize   : pagingSize
						,page         : page
					};

					
					/* ___ajaxCall("/interface/eReceiptList.do", objParam, false, "json", "post"
							,function(result) {
								
								if (result.result_code == "SUCCESS") {
									
									$(".card_use_info_tbl tbody").empty();
									...
									if(jsonData.totalCnt > 0){
										$("#cardUseHist").tmpl(receiptList, {
											 getSeq : function () {
												return nSeq--;
											}
											,getCardRegNumber : function () {
												return cardRegNumber;
											}
										}).appendTo(".card_use_info_tbl tbody");
										
										$("ul.pager").empty();

										$replyPage     = page;
										$mycTotalCount = jsonData.totalCnt;
										$mycPageSize   = pagingSize;
										...
										$("ul.pager").paging({
											...
											,onclick : function (e, page) {
												makeReceiptPage(page);
											}
										});

									}else{
										$(".card_use_info_tbl tbody").html('<tr><td colspan="6">데이터가 없습니다.</td></tr>');
										$("ul.pager").html("");
									}
				...
				function setEvent(){
					...
					/* 20181106 전자영수증 기간 확대 기준날짜(2016년12월03일) 이전 선택 불가 처리 by kbs */
					var dateCheck = searchDateCheck();
					var minDateSet = "";
					
					/* true : 16년12월03일 이전 선택불가, false 현재날짜 -5year까지 선택 가능 */
					if(dateCheck == true){
						minDateSet = '2016-12-03';
					}else{
						minDateSet = '-5Y +1D';
					}
					
					$('#receiptCal1, #receiptCal2').datepicker({
					...
					//기간별 radio box 이벤트
					$("input[name=pickPeriod]").change(function() {
						var strStartDt = "";
						...
						switch ($(this).val()) {
							...
							case "Y":
								...
								break;
							case "P":
								strStartDt = $("#receiptCal1").val();
								break;
						}
						
						$("#receiptCal1").val(strStartDt);
						$("#receiptCal2").val(strEndDt);
					});
					
					$("#receiptCal1, #receiptCal2").on("change", function () {
						$("input[name=pickPeriod][value=P]").prop("checked", true);
					});
					
					$('#receiptCal1').datepicker("option", "onClose", function ( selectedDate ) {
				        $("#receiptCal2").datepicker( "option", "minDate", selectedDate );
				    });
					
				}
				
				/* 20181106 전자영수증 기간 확대 기준날짜(2016년12월03일) 이전 선택 불가 처리 and 5년이하 선택 불가 처리 by kbs */
				function searchDateCheck(){
					//조회 불가 날짜 설정
					var compDate = "20161203000000";

					//서버 현재 시간 -5년 type number
					today = serverTime - 50000000000;
					
					// 시간비교
					if ( (eval(today) <= eval(compDate)) )  {  
					    return true;
					} else {
						return false;
					}
