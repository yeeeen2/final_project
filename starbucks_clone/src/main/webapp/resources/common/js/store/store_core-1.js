    ...
    $.storemap = {
        ...
        createMapContents : function($rows, $image, $offset, $vo)
        {
        	...
        	if ( $rows.length > 0)
        	{
        		...
        		if ($page.indexOf('store_star_field') > -1) {
        ...
        setIncon : function(theme, type)
        {
        	if( theme != null )
        	{
            	if ( theme.indexOf(type) >-1 )
            	{
            		...
            		if( type == "T48") {	return "<img src='//image.istarbucks.co.kr/upload/common/img/icon/icon_eco_service.png' />"; }		//20210407 '에코매장' 서비스 추가
        ...
        setSidoGugun : function()
        {
			...
			__ajaxCall("${pageContext.request.contextPath}/store/getSidoList", {}, true, "JSON", "POST",
					function (_response) 
					{
        				if ( _response.list.length > 0 )
        				{
    						...
    						**/
            				...
            				$(".set_sido_cd_btn").on("click", function(e){
            					...
            					{
                					__ajaxCall("${pageContext.request.contextPath}/store/getGugunList", {"sido_cd":sido}, true, "json", "post",
                							function (_response) 
                							{
                								$(".gugun_arae_box").html("");
